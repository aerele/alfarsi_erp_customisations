...
refresh(frm) {
    frm.add_custom_button(__('Automate Registration'), function () {

        const to_register = (frm.doc.medical_devices || [])
            .filter(r => r.register_with_moh);

        if (!to_register.length) {
            frappe.msgprint(__('Please tick "Register with MOH" for at least one item.'));
            return;
        }

        frappe.call({
            method: 'alfarsi_erp_customisations.alfarsi_erp_customisations.doctype.moh_automation.moh_automation.trigger_moh',
            args: {
                moh_doc: frm.doc.name
            },
            callback() {
                frappe.msgprint(__('MOH automation started. The PKI machine will process selected items.'));
                frm.reload_doc();
            }
        });
