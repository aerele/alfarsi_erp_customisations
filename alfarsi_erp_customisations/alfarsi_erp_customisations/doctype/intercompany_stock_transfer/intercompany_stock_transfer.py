...
class IntercompanyStockTransfer(Document):
	def validate(self):
		...
		for accounts in self.table_npbv:
			...
			if frappe.db.get_value("Account",accounts.stock_in_account_booking,"company") != accounts.company:
				error +=f"<br >Row {accounts.idx} Account {accounts.stock_in_account_booking} is not in Company {accounts.company} <br>"
			if frappe.db.get_value("Account",accounts.stock_out_account_booking,"company") != accounts.company:
