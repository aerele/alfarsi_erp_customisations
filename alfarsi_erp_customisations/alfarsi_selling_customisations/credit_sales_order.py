...
class SellingCreditControl:
        ...
        if customer_docs.credit_limit == 0 and customer_docs.credit_days == 0:
            ...
            if customer_docs.credit_limit > 0:
                ...
                self.limit_tolerance = (
                    customer_docs.credit_limit_tolerance
                    if customer_docs.credit_limit_tolerance > 0
                    else settings.default_credit_limit_tolerance
                )
                self.validate_limit = True
            else:
                self.validate_limit = False
            ...
            self.approval_role = (
                customer_docs.customer_approval_role
                if customer_docs.customer_approval_role is not None
                else settings.default_approval_role
            )
    ...
    def validate_credit_days(self):
        ...
        if total_days_passed <= self.customer_credit_days:
            ...
            self.notify_limit_manager()
            frappe.msgprint(
                "Credit days exceeded. Chief Mentors Approval required to proceed.\nThe account is currently on credit hold as it has exceeded the approved credit period. Kindly arrange payment of the outstanding amount to proceed further."
            )
        else:
            self.days_status = "exceeded"
            frappe.throw(
                "The account is currently on credit hold as it has exceeded the approved credit limit days. Kindly arrange payment of the outstanding amount to proceed further."
            )

    def notify_limit_manager(self):
        approval_role = frappe.db.get_value(
            "Customer", self.customer, "customer_approval_role"
        )
        if not approval_role:
            approval_role = frappe.db.get_single_value(
                "Credit Control Settings", "default_approval_role"
            )
        if not approval_role:
        ...
        if emails:
            message = frappe.render_template(
                "templates/emails/credit_limit.html",
                {
                    ...
                    "outstanding_amount": self.customer_outstanding,
                    "credit_limit": self.customer_credit_limit,
                    "credit_difference": self.order_amount
    ...
    def check_submit_permissions(self):
        ...
        if "exceeded" in statuses:
            if not ("Finance Manager" in roles or "Chief Mentor" in roles):
                frappe.throw(
                    "Only Finance Manager or Chief Mentor can submit this order as the credit limit or days are exceeded beyond the allowed tolerance."
                )
            return
        if "within" in statuses:
            if not ("Chief Mentor" in roles or "Finance Manager" in roles):
                frappe.throw(
                    "Only Chief Mentor or Finance Manager can submit this order as the credit limit or days are exceeded but within the allowed tolerance."
                )
            return
        return
